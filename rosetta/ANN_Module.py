...
class ANN(object):
    ...
    def parse_transfer_funcs(self, nl, tr):
        ...
        assert (
            nl <= ntr
        ), "number of layers must be smaller or equal to number of transfer functions"
    ...
    def from_stream(
        ...
        ann = ANN(
            nlayer=nlayer, transfers=transfers
    ...
    def purelin(self, x):
        y = N.copy(
            x
        )  # copy because we don't know what we're doing with x next outside this function.
        return y
...
class REPLICA(object):
    ...
    @property
    def ncnv(self):
        nv = N.count_nonzero(N.array(self.pat == 0, dtype=int))
        nc = len(self.pat)
        return (nc, nv)
...
class PS(object):
    ...
    def parse_query(self, cursor):
        ...
        xmax = N.zeros((self.nvar,), dtype=float)
        ymin = N.zeros((self.nvar,), dtype=float)
    ...
    def check_data(self, data_in):
        ...
        if ssc_set.issubset(vnset):
            ...
            res_ssc = N.logical_and(
                N.greater_equal(ssc_sum, 99.0), N.less_equal(ssc_sum, 101.0)
            )  # must be between these values
...
class ANN_MODEL(object):
    ...
    @property
    def nout(self):
        return self.ann_sequence[0].nout  # hack! ann_sequence[0] should exist
    ...
    def __init__(self, model_id, db):
        ...
        self.ann_sequence = [
            ANN.from_stream(
                io.BytesIO(ann_bin),
                nlayer=nlayer,
                transfers=[nhid1_transfer, nhid2_transfer, nout_transfer],
            )
            for (
                hash_id,
                seq,
    ...
    def predict(self, data):
        ...
        if nsamp == nsamp_valid:
            ...
            res_fin = N.ones((self.nmodel, self.nout, nsamp), dtype=float) * -9.9
...
class PTF_MODEL(object):
    ...
    def predict(self, data_in, sum_data=True):
        ...
        if sum_data:
            ...
            sum_res_mean = N.zeros((nout_total, nsamp), dtype=float)
            sum_res_std = N.zeros((nout_total, nsamp), dtype=float)
            sum_res_skew = N.zeros((nout_total, nsamp), dtype=float)
            sum_res_kurt = N.zeros((nout_total, nsamp), dtype=float)
